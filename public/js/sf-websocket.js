...
(function () {
  ...
  var _newChannel = document.getElementById('addChannel');
  ...
  /**
   * Desactivate a given channel
   *
   * @param {string} channel - Channel name
   */
  var desactivateChannel = function desactivateChannel(channel) {
    // Initialize
    var _channel = '#' + channel;
    ...
    // Activate parent list item
    _chanLink[0].parentElement.classList.remove('active');
  };
  ...
  var addMessageToChannel = function(message) {
    ...
    _receiver.insertAdjacentHTML("beforeend", '<li class="' + liClass + '">' +
        '<strong>' + _message.user + '</strong>: ' + _message.message +
      '</li>');
    _receiver.scrollTop = _receiver.scrollHeight-_receiver.offsetHeight;
  };
   ...
   */
  ws.onerror = function () {
    botMessageToGeneral('Sorry, nobody is online... Please come back later', 'error');
    disableFormAndWsConnection();
  };

  // Bind form events
  _newChannel.onclick = function() {
      var channel = "essai";
      addChannel(channel);
      activateChannel(channel);
      desactivateChannel(defaultChannel);
